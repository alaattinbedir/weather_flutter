...
class DioClient {
  ...
  Future<dynamic> get(String baseUrl, String api) async {
    ...
    try {
      var response = await Dio().get(baseUrl + api).timeout(const Duration(seconds: timeOutDuration));
  ...
  Future<dynamic> post(String baseUrl, String api, dynamic payloadObj) async {
    ...
    try {
      var response = await Dio().post(baseUrl + api, data: payload).timeout(const Duration(seconds: timeOutDuration));
      return _processResponse(response);
    } on SocketException {
      throw FetchDataException('No Internet connection', uri.toString());
    } on TimeoutException {
